...
"""
type(html_doc) #It's just one big string
...
body = b.find(name='body') # Finds the 'body' tag and everything inside of it.
b.find(name='ul')
type(body) #tag
...
hobbies = b.find_all(name='li')

for i in hobbies:
    print i.text

# 3. Extract Brandon's hobby that has the id "my favorite".
favorite = b.find(name='li', attrs={'id':'my favorite'})
...
# Now let's talk about 'attrs'
# Beautiful soup also allows us to choose tags with specific attributes
title = c.find(name='span', attrs={'class':'itemprop', 'itemprop':'name'})
title.text
title['class']

star_rating = c.find(name='div', attrs={'class':'titlePageSprite star-box-giga-star'})
star_rating.text
...
searchng within it.
'''
infobar = c.find_all(name='div', attrs={'class':'infobar'})
#rating
rating = c.find(name='meta', attrs={'itemprop':'contentRating'})['content']
#duration
duration = c.find(name='time', attrs={'itemprop':'duration'}).text
#genre
genre = c.find(name='span', attrs={'itemprop':'genre'}).text
...
actors_raw = c.find(name='div', attrs={'itemprop':'actors', 'itemscop':'', 'itemtype':'http://schema.org/Person'})
# Now let's find all of the occurences of the "span" with "itemprop" "name",
# meaning the tags with actors' and actresses' names.
actors = actors_raw.findAll(name='span', attrs={'itemprop':'name'}) #We use findAll b/c actors_raw is a BeautifulSoup ResultSet
